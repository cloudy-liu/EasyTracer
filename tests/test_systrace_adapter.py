...
class TestSystraceAdapter(unittest.TestCase):
    ...
    def test_run_systrace_success(self, mock_run):
        # Mock _import_and_run_systrace to avoid running the complex script
        with patch.object(
            self.adapter, "_import_and_run_systrace", return_value="Tracing complete"
        ) as mock_import_run:
    ...
    def test_get_categories_success(self, mock_run):
        ...
        with patch.object(
            self.adapter, "_import_and_run_systrace", return_value=mock_stdout
        ):
