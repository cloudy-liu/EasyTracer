        ...
        if options.boot:
            ...
            if device_sdk_version <= 22:
                print('--boot option does not work on the device SDK '
                      ...
                      'is %d.' % device_sdk_version, file=sys.stderr)
                sys.exit(1)
...
class AtraceAgent(systrace_agent.SystraceAgent):
    ...
    def _collect_trace_data(self):
        ...
        if self._adb.returncode != 0:
            print('The command "%s" returned error code %d.' %
                  (' '.join(self._tracer_args), self._adb.returncode), file=sys.stderr)
            sys.exit(1)
    ...
    def start(self):
        try:
            ...
            print('The command "%s" failed with the following error:' %
                  ' '.join(tracer_args), file=sys.stderr)
...
class FileReaderThread(threading.Thread):
    ...
    def __init__(self, file_object, output_queue, text_file, chunk_size=-1):
        ...
        Args:
            ...
            output_queue: A Queue object that will receive the data
...
def extract_tgids(trace_text):
    ...
    text = trace_text.splitlines()
    for line in text:
        result = re.match(r'^/proc/([0-9]+)/task/([0-9]+)', line)
        if result:
...
def strip_and_decompress_trace(trace_data):
    ...
    Args:
        trace_data: The trace data returned by atrace (can be str or bytes).
    Returns:
    ...
    if trace_bytes.startswith(b'\r\n'):
        trace_bytes = trace_bytes.replace(b'\r\n', b'\n')
    elif trace_bytes.startswith(b'\r\r\n'):
        # On windows, adb adds an extra '\r' character for each line.
        trace_bytes = trace_bytes.replace(b'\r\r\n', b'\n')

    # Skip the initial newline.
    if trace_bytes and trace_bytes[0:1] == b'\n':
        trace_bytes = trace_bytes[1:]

    # Check if this is plain text or compressed
    if not trace_bytes.startswith(TRACE_TEXT_HEADER.encode('utf-8')):
        # No header found, so assume the data is compressed.
        try:
            trace_bytes = zlib.decompress(trace_bytes)
        except zlib.error:
            # If decompression fails, data might already be plain text
            pass

    # Convert to string
    trace_data = trace_bytes.decode('utf-8', errors='replace')
